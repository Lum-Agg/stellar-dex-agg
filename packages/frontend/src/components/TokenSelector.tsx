  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="font-medium text-[15px] text-[var(--text-primary)]">{selected.symbol}</span>
      ...
      {open && typeof window !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/75 backdrop-blur-[2px]" onClick={() => { setOpen(false); setSearch(''); }}>
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl w-full max-w-md mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
              <h3 className="text-[15px] font-semibold text-[var(--text-primary)]">Select a token</h3>
              <button onClick={() => { setOpen(false); setSearch(''); }} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]">
              ...
              {filtered.slice(0, 50).map(token => {
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/[0.03] transition-colors ${
                    token.id === selected.id ? 'bg-white/[0.03] border border-[var(--border)]' : ''
                  }`}
                >
                  <TokenIcon token={token} size={36} />
                  <div className="text-left min-w-0 flex-1">
                    <div className="text-sm font-semibold text-[var(--text-primary)]">{token.symbol}</div>
